...
from cnn_lstm import ParallelCNNLSTMModel  # Import your model class from the correct path
...
SEQ_LENGTH = 500
NUM_EPOCHS = 5
EXPERIMENT_NAME = "PARALLEL_CNN_LSTM_OPTIMIZATION"
PIN_MEMORY = True
LOAD_MODEL = False
...
DURATION = 2*3600  # Set the timeout duration in seconds (e.g., 3600 seconds = 1 hour)
N_TRIALS = 100
conv_filters_mapping = {
    0: [64, 128, 256],
    1: [64, 128, 256, 512]
}
...
def suggest_hyperparameters(trial):
    conv_filters_index = trial.suggest_categorical('conv_filters_index', [0, 1])
    lstm_hidden_size = trial.suggest_int('lstm_hidden_size', 32, 256)
    lstm_num_layers = trial.suggest_int('lstm_num_layers', 1, 4)
    dropout = trial.suggest_float('dropout', 0.0, 0.5)
    lr = trial.suggest_float('lr', 1e-5, 1e-3, log=True)
    weight_decay = trial.suggest_float('weight_decay', 1e-6, 1e-2, log=True)
    batch_size = trial.suggest_int('batch_size', 32, 128)
    fc_neurons1 = trial.suggest_int('fc_neurons1', 128, 1024)
    fc_neurons2 = trial.suggest_int('fc_neurons2', 64, 512)
    activation_choice = trial.suggest_categorical('activation', ['ReLU', 'GELU'])

    conv_filters = conv_filters_mapping[conv_filters_index]

    print(f"Suggested hyperparameters: \n{pformat(trial.params)}")
    return conv_filters, lstm_hidden_size, lstm_num_layers, dropout, lr, weight_decay, batch_size, fc_neurons1, fc_neurons2, activation_choice
...
def objective(trial, experiment, device, options=None):
    ...
    with mlflow.start_run(experiment_id=experiment.experiment_id) as run:
        ...
        conv_filters, lstm_hidden_size, lstm_num_layers, dropout, lr, weight_decay, batch_size, fc_neurons1, fc_neurons2, activation_choice = suggest_hyperparameters(trial)
        active_run = mlflow.active_run()
        print(f"Starting run {active_run.info.run_id} and trial {trial.number}")

        conv_filters_index = next(key for key, value in conv_filters_mapping.items() if value == conv_filters)
        mlflow.log_param("conv_filters_index", conv_filters_index)
        ...
        input_size = SEQ_LENGTH
        num_classes = 4
        train_loader, val_loader, _, _, _, _ = get_loaders_no_sss(train_dir=TRAIN_DIR,
                                                            test_dir=TEST_DIR, 
...
def main():
    ...
    if options["save_model"]:
        ...
        best_model = ParallelCNNLSTMModel(
            input_size=SEQ_LENGTH,
            input_size_lstm=1,
            num_classes=NUM_CLASSES,
            conv_filters=conv_filters_mapping[best_model_params['conv_filters_index']],
            lstm_hidden_size=best_model_params['lstm_hidden_size'],
            lstm_num_layers=best_model_params['lstm_num_layers'],
            fc_neurons=[best_model_params['fc_neurons1'], best_model_params['fc_neurons2']],
            dropout=best_model_params['dropout'],
            activation=nn.ReLU() if best_model_params['activation'] == 'ReLU' else nn.GELU()
        ).to(DEVICE)
        ...
        mlflow.pytorch.log_model(best_model, "best_model", registered_model_name='best_parallel_cnn_lstm_model')
        print(f"Best model saved as {model_path}")
